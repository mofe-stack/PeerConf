...
- `PROBE_EVERY`: how many tokens between completion probes. 4096 on aime25 and hmmt25,
  512 on math500. the probe looks stops on deepseek and qwen's end of thinking marker `</think>`, so running gpt-oss means changing that
  to its own marker of `<|end|>` in cell 2. set it to 0 to turn probes off entirely 
